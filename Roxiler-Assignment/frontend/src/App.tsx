...
function App() {
  ...
  return (
    <div className="bg-blue-200 w-screen p-8 m-auto h-full">
      <h2 className="w-[150px] h-[150px] text-lg font-bold flex items-center justify-center rounded-full text-center text-gray-600 bg-white e m-auto">Transaction Dashboard</h2>
      <div className="w-screen">
        <div className="max-w-[90%] mx-auto my-4 h-12 flex flex-row justify-between px-11">
          <input type="text" value={search} className="w-[20%] bg-yellow-200 text-neutral-900 font-semibold rounded-md px-4 py-1 outline-none" placeholder="Search Transaction" onChange={(e) => { setSearch(e.target.value) }} />
          <select value={month} className="bg-yellow-200 rounded-md w-[14%] px-2 h-[90%]" onChange={(e) => setMonth(e.target.value)} defaultValue={"mar"} name="month" id="month">
      ...
      <div style={{
          ...
          Created by <strong>Bhagyashri Nage</strong>
        </div>
